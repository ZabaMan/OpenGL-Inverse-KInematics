...
void Model::ExtractBoneWeightForVertices(std::vector<Vertex>& vertices, aiMesh* mesh, const aiScene* scene)
{
    for (int boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex)
    {
        ...
        if (m_BoneInfoMap.find(boneName) == m_BoneInfoMap.end())
        {
            ...
            m_BoneCounter++;
            m_Bones.push_back(Bone(boneName, boneID, newBoneInfo.offset));
        }
        else
        {
...
std::vector<glm::mat4> Model::GetBoneTransforms()
{
	...
	for(Bone b: m_Bones)
	{
        boneTransforms.push_back(b.GetLocalTransform());
	}
    return boneTransforms;
}

void Model::CalculateBoneTransform(glm::mat4 localTransform, string boneName)
{
    /*std::string nodeName = boneName;
    glm::mat4 nodeTransform = localTransform;*/

    Bone* Bone = FindBone(boneName);

    if (Bone)
    {
        Bone->Update(localTransform);
